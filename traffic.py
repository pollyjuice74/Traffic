...
def load_data(data_dir):
    ...
    images = []
    labels = []
    for category in range(NUM_CATEGORIES): # NUM_CATEGORIES
...
def get_model():
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)), # Convolution layer
    ...
    model.save("models/tree_ConvPoolDenseDropout05_nn.h5")

    return model
    """
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(units=10, activation='relu', input_shape=((IMG_WIDTH,IMG_HEIGHT, 3))),
